...
- 权限缺失时提供明确引导
- 未授权时无法开始录音
- 首次按全局快捷键时，若麦克风权限未决定，自动弹出系统授权
- 麦克风权限已拒绝或受限时，自动打开系统麦克风隐私设置
...
- 设置页可展示辅助功能权限状态
- 权限缺失时提供明确引导
- 首次按全局快捷键且麦克风权限通过后，若辅助功能未授权，自动触发系统辅助功能授权引导
- 未授权时无法开始录音，不进入录音 HUD，并返回明确权限错误
- 权限引导期间重复按快捷键不会创建多个 session 或重复触发弹窗
...
作为用户，我希望将 MacBook 的 `Fn`／🌐 键设为全局录音快捷键，以便单指完成按一次开始、再按一次结束的录音操作。

验收标准：

- 设置页可录制并显示 `Fn` 单键快捷键，保存后立即生效，重启后加载一致
- `Fn` 快捷键在 Typoless 前台及其他应用前台均可触发；按住不重复触发
- `Fn` 与修饰键的组合同样可录制和触发；按 F1–F12 等功能键不会误触发已绑定的 `Fn` 快捷键
- 现有普通组合键与左右侧纯修饰键快捷键行为不回归
- 含 `Fn` 的配置与旧版 JSON 结构向后兼容
- 选中 `Fn` 时设置页提示：若系统「按下 🌐 键时」配置了动作，需在系统设置中改为「无操作」
