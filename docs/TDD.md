...
- 在设置页中展示状态
- `PermissionsManager.prepareForVoiceInputStart()` 作为语音输入启动前的统一权限编排入口
- 首次按快捷键时，若麦克风权限为未决定，则调用系统授权弹窗
- 若麦克风权限已拒绝或受限，则打开系统麦克风隐私设置并返回权限错误
- 未授权时禁止开始录音，不创建录音 session

### 12.2 辅助功能权限

- 在设置页中展示状态
- 麦克风权限通过后，再检查辅助功能权限
- 未授权时调用 `AXIsProcessTrustedWithOptions(prompt: true)` 触发系统授权引导
- 未授权时禁止开始录音，不进入录音 HUD，并给出明确提示
- 权限准备期间 `SessionCoordinator` 进入 `preparing` 状态；重复快捷键不会再次触发权限准备或创建多个 session
