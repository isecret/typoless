...
首版产品形态不是系统级输入法，而是 `菜单栏常驻应用`。用户通过全局快捷键触发录音（支持长按模式和按下切换模式），录音结束后自动完成：
...
- 应用形态：菜单栏常驻应用
- 交互方式：单一全局快捷键，支持两种录音触发方式
  - `长按录音`：按住快捷键录音，松开处理
  - `按下切换`（默认）：按一次开始，再按一次结束
- 单次录音上限：`30 秒`
- 默认输出：`LLM 润色版`
...
1. 在任意应用中聚焦输入区域
2. 按下快捷键开始录音（按下切换模式，默认），或按住快捷键开始录音（长按模式）
3. 再次按下快捷键结束录音（按下切换模式），或松开快捷键结束录音（长按模式）
4. 提交音频到腾讯云 ASR
5. 获取原始转写文本
6. 调用 LLM 做纠错与轻度书面化
7. 将最终文本注入当前焦点应用
